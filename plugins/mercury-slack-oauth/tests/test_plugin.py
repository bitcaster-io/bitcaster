...
env = Env(MERCURY_SLACK_OAUTH_USERNAME='',
          MERCURY_SLACK_OAUTH_PASSWORD='',
          MERCURY_SLACK_OAUTH_RECIPIENT='',
          )
...
def before_record_request(request):
    ...
    for e in [env('MERCURY_SLACK_OAUTH_USERNAME', str),
              env('MERCURY_SLACK_OAUTH_PASSWORD', str),
              env('MERCURY_SLACK_OAUTH_RECIPIENT', str)]:
...
def subscription():
    ...
    channel = Mock(application=application,
                   config={'username': env('MERCURY_SLACK_OAUTH_USERNAME', str),
                           'password': env('MERCURY_SLACK_OAUTH_PASSWORD', str)
                           })
    event = Mock(application=application)

    return Mock(subscriber=user,
                event=event,
                config={'recipient': env('MERCURY_SLACK_OAUTH_RECIPIENT', str)},
                channel=channel)


def test_validate_subscription(subscription):
    d = SlackOAuth(subscription.channel)
...
def test_send(subscription):
    with vcr.use_cassette('test_send.yaml'):
        ...
        assert d.emit(subscription,
                      'subject',
                      'Mercury is on SlackOAuth...enjoy') == 1
