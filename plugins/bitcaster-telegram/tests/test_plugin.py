...
from bitcaster_telegram import Telegram

from bitcaster.exceptions import ValidationError

env = Env(BITCASTER_BITCASTER_TELEGRAM_USERNAME='',
          BITCASTER_BITCASTER_TELEGRAM_PASSWORD='',
          BITCASTER_BITCASTER_TELEGRAM_RECIPIENT='',
          )
...
def before_record_request(request):
    ...
    for e in [env('BITCASTER_BITCASTER_TELEGRAM_USERNAME', str),
              env('BITCASTER_BITCASTER_TELEGRAM_PASSWORD', str),
              env('BITCASTER_BITCASTER_TELEGRAM_RECIPIENT', str)]:
...
def subscription():
    ...
    channel = Mock(application=application,
                   config={'username': env('BITCASTER_BITCASTER_TELEGRAM_USERNAME', str),
                           'password': env('BITCASTER_BITCASTER_TELEGRAM_PASSWORD', str)
                           })
    event = Mock(application=application)

    return Mock(subscriber=user,
                event=event,
                config={'recipient': env('BITCASTER_BITCASTER_TELEGRAM_RECIPIENT', str)},
                channel=channel)


def test_validate_subscription(subscription):
    d = Telegram(subscription.channel)
...
def test_send(subscription):
    with vcr.use_cassette('test_send.yaml'):
        ...
        assert d.emit(subscription,
                      'subject',
                      'Bitcaster is on Telegram...enjoy') == 1
