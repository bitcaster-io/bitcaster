...
def test_flow(config: SocialProvider, client, mocked_responses, monkeypatch):
    STATE = "-state-"
    CODE = "-code-"
    ...
    assert resp.url == expected_authorize_url
    ...
    assert (
        resp.wsgi_request.user.email,
        resp.wsgi_request.user.first_name,
        resp.wsgi_request.user.last_name,
    ) == (
