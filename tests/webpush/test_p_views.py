...
def test_push_serviceworker(django_app, application: "Application"):
    url = reverse("webpush:service_worker", args=[application.slug])
    res = django_app.get(url)
    assert res.status_code == 200


def test_push_ask(django_app, push_assignment: "Assignment", monkeypatch):
...
def test_push_subscribe(django_app, push_assignment: "Assignment", monkeypatch):
    secret = sign(push_assignment)
...
def test_push_subscribe_404(django_app, push_assignment: "Assignment", monkeypatch):
    secret = sign(push_assignment)
...
def test_push_unsubscribe(django_app, push_assignment: "Assignment", monkeypatch):
    secret = sign(push_assignment)
...
def test_push_data(django_app, push_assignment: "Assignment", monkeypatch):
    secret = sign(push_assignment)
