...
This is the default behavior. The notification is sent to the members of the **Distribution List** linked to the notification.
...
*   **API interaction**:
    *   `limit_to` narrows delivery to list members whose address value (email, phone, etc.) appears in the list.
    *   `filters` is **ignored**.
